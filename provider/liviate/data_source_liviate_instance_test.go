...
func TestAccInstanceDataSource_basic(t *testing.T) {
	resourceName := "liviate_instance.my_instance"
	datasourceName := "data.liviate_instance.my_instance_test"
...
const testAccInstanceDataSourceConfig_basic = `
	resource "liviate_instance" "my_instance" {
		name             = "server-a"
		service_offering = "Small Instance"
		template         = "CentOS 5.6 (64-bit) no GUI (Simulator)"
		zone             = "Sandbox-simulator"
	  }
	  data "liviate_instance" "my_instance_test" {
		filter {
		name = "display_name" 
		value = "server-a"
	  }
		depends_on = [
		liviate_instance.my_instance
	  ]
	}
`
