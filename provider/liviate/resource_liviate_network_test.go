		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"liviate_network.foo", "project", "terraform"),
				),
			},
		},
	})
...
func TestAccCloudStackNetwork_import(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				ResourceName:      "liviate_network.foo",
				ImportState:       true,
...
resource "liviate_network_acl" "bar" {
  name = "bar"
  vpc_id = liviate_vpc.foo.id
}
...
resource "liviate_network" "foo" {
  ...
  network_offering = "DefaultIsolatedNetworkOfferingForVpcNetworks"
  vpc_id = liviate_vpc.foo.id
  acl_id = liviate_network_acl.bar.id
  zone = liviate_vpc.foo.zone
}`
