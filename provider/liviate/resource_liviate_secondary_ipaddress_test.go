		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackSecondaryIPAddressExists(
						"liviate_secondary_ipaddress.foo", &ip),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"liviate_secondary_ipaddress.foo", "ip_address", "10.1.1.123"),
				),
			},
		},
	})
...
func testAccCheckCloudStackSecondaryIPAddressDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "liviate_secondary_ipaddress" {
			continue
		}
...
resource "liviate_secondary_ipaddress" "foo" {
