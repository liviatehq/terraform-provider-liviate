		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackAutoscaleVMProfileExists("liviate_autoscale_vm_profile.foo", &vmProfile),
					testAccCheckCloudStackAutoscaleVMProfileBasicAttributes(&vmProfile),
					resource.TestCheckResourceAttr(
						"liviate_autoscale_vm_profile.foo", "zone", "Sandbox-simulator"),
...

			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackAutoscaleVMProfileExists(
						"liviate_autoscale_vm_profile.foo", &vmProfile),
...
func testAccCheckCloudStackAutoscaleVMProfileDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "liviate_autoscale_vm_profile" {
			continue
		}
...
resource "liviate_autoscale_vm_profile" "foo" {
