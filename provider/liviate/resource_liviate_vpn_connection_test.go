		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackVPNConnectionExists(
						"liviate_vpn_connection.foo-bar", &vpnConnection),
					testAccCheckCloudStackVPNConnectionExists(
						"liviate_vpn_connection.bar-foo", &vpnConnection),
				),
			},
		},
	})
...
func testAccCheckCloudStackVPNConnectionDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "liviate_vpn_connection" {
			continue
		}
...
resource "liviate_vpn_connection" "foo-bar" {
  customer_gateway_id = liviate_vpn_customer_gateway.foo.id
  vpn_gateway_id = liviate_vpn_gateway.bar.id
}

resource "liviate_vpn_connection" "bar-foo" {
  customer_gateway_id = liviate_vpn_customer_gateway.bar.id
  vpn_gateway_id = liviate_vpn_gateway.foo.id
}`
