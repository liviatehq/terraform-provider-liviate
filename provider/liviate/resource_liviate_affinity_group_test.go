		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackAffinityGroupExists("liviate_affinity_group.foo", &affinityGroup),
...
func TestAccCloudStackAffinityGroup_import(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				ResourceName:      "liviate_affinity_group.foo",
				ImportState:       true,
...
func testAccCheckCloudStackAffinityGroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "liviate_affinity_group" {
			continue
		}
...
resource "liviate_affinity_group" "foo" {
