		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackTrafficTypeExists(
						"liviate_traffic_type.foo", &trafficType),
					testAccCheckCloudStackTrafficTypeBasicAttributes(&trafficType),
					resource.TestCheckResourceAttrSet(
						"liviate_traffic_type.foo", "traffic_type"),
					resource.TestCheckResourceAttr(
						"liviate_traffic_type.foo", "kvm_network_label", "cloudbr0"),
				),
			},
		},
	})
...
func TestAccCloudStackTrafficType_import(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				ResourceName:      "liviate_traffic_type.foo",
				ImportState:       true,
...
func testAccCheckCloudStackTrafficTypeDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "liviate_traffic_type" {
			continue
		}
...
resource "liviate_traffic_type" "foo" {
