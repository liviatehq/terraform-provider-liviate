...
func resourceCloudStackInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	// Check if the root disk size has changed and if so, resize the root disk
	if d.HasChange("root_disk_size") {
		log.Printf("[DEBUG] Root disk size changed for %s, starting update", name)

		// Find the root volume of the instance
		p := cs.Volume.NewListVolumesParams()
		p.SetType("ROOT")
		p.SetVirtualmachineid(d.Id())
		...
		if len(l.Volumes) != 1 {
			return fmt.Errorf("Failed to find root disk of instance: %s", name)
		}

		// Resize the root disk to the new size (in GiB)
		rp := cs.Volume.NewResizeVolumeParams(l.Volumes[0].Id)
		rp.SetSize(int64(d.Get("root_disk_size").(int)))
		_, err = cs.Volume.ResizeVolume(rp)
		if err != nil {
			return fmt.Errorf(
				"Error resizing root disk for instance %s: %s", name, err)
		}
	}
