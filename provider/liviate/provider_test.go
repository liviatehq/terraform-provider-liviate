...
var cloudStackTemplateURL = os.Getenv("LIVIATE_TEMPLATE_URL")

func init() {
	testAccProvider = Provider()
	testAccProviders = map[string]*schema.Provider{
		"liviate": testAccProvider,
	}

	testAccMuxProvider = map[string]func() (tfprotov6.ProviderServer, error){
		"liviate": func() (tfprotov6.ProviderServer, error) {
...
resource "liviate_zone" "zone_resource"{
	name       		= "TestZone"
  	dns1       		= "8.8.8.8"
  ...
  data "liviate_zone" "zone_data_source"{
    filter{
    	name 	=	"name"
    	value	=	liviate_zone.zone_resource.name
    }
  }
  `
...
func testAccPreCheck(t *testing.T) {
	if v := os.Getenv("LIVIATE_API_URL"); v == "" {
		t.Fatal("LIVIATE_API_URL must be set for acceptance tests")
	}
	if v := os.Getenv("LIVIATE_API_KEY"); v == "" {
		t.Fatal("LIVIATE_API_KEY must be set for acceptance tests")
	}
	if v := os.Getenv("LIVIATE_SECRET_KEY"); v == "" {
		t.Fatal("LIVIATE_SECRET_KEY must be set for acceptance tests")
	}
}
