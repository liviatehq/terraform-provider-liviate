		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackNetworkServiceProviderExists(
						"liviate_network_service_provider.foo", &provider),
					testAccCheckCloudStackNetworkServiceProviderBasicAttributes(&provider),
					resource.TestCheckResourceAttr(
						"liviate_network_service_provider.foo", "name", "VirtualRouter"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackNetworkServiceProviderExists(
						"liviate_network_service_provider.security_group", &provider),
					testAccCheckCloudStackNetworkServiceProviderSecurityGroupAttributes(&provider),
					resource.TestCheckResourceAttr(
						"liviate_network_service_provider.security_group", "name", "SecurityGroupProvider"),
				),
			},
		},
	})
...
func TestAccCloudStackNetworkServiceProvider_import(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				ResourceName:      "liviate_network_service_provider.foo",
				ImportState:       true,
...
func testAccCheckCloudStackNetworkServiceProviderDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "liviate_network_service_provider" {
			continue
		}
...
resource "liviate_physical_network" "foo" {
  name = "terraform-physical-network"
  zone_id = liviate_zone.foo.id
  broadcast_domain_range = "ZONE"
  isolation_methods = ["VLAN"]
}

resource "liviate_network_service_provider" "foo" {
  name = "VirtualRouter"
  physical_network_id = liviate_physical_network.foo.id
...
resource "liviate_network_service_provider" "security_group" {
