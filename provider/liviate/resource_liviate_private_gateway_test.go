		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackPrivateGatewayExists(
						"liviate_private_gateway.foo", &gateway),
...
func TestAccCloudStackPrivateGateway_import(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				ResourceName:      "liviate_private_gateway.foo",
				ImportState:       true,
...
func testAccCheckCloudStackPrivateGatewayDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "liviate_private_gateway" {
			continue
		}
...
resource "liviate_network_acl" "foo" {
  ...
  depends_on = ["liviate_vpc.foo"]
}

resource "liviate_private_gateway" "foo" {
  gateway = "10.1.1.254"
  ...
  depends_on = ["liviate_vpc.foo","liviate_network_acl.foo"]
}`
