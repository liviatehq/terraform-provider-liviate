		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackQuotaTariffExists("liviate_quota_tariff.test", &quotaTariff),
					resource.TestCheckResourceAttr("liviate_quota_tariff.test", "name", "Test CPU Tariff"),
					resource.TestCheckResourceAttr("liviate_quota_tariff.test", "usage_type", "1"),
					resource.TestCheckResourceAttr("liviate_quota_tariff.test", "value", "0.05"),
				),
			},
		},
	})
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackQuotaTariffExists("liviate_quota_tariff.test", &quotaTariff),
					resource.TestCheckResourceAttr("liviate_quota_tariff.test", "name", "Updated CPU Tariff"),
					resource.TestCheckResourceAttr("liviate_quota_tariff.test", "value", "0.10"),
					resource.TestCheckResourceAttr("liviate_quota_tariff.test", "description", "Updated description"),
				),
			},
		},
	})
...
func TestAccCloudStackQuotaTariff_import(t *testing.T) {
	resourceName := "liviate_quota_tariff.test"
...
func testAccCheckCloudStackQuotaTariffDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "liviate_quota_tariff" {
			continue
		}
...
func testAccCloudStackQuotaTariff_basic() string {
	return `
resource "liviate_quota_tariff" "test" {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackQuotaTariffExists("liviate_quota_tariff.test", &quotaTariff),
					resource.TestCheckResourceAttr("liviate_quota_tariff.test", "activation_rule", "serviceOffering.id == 'test-so-123'"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackQuotaTariffExists("liviate_quota_tariff.test", &quotaTariff),
					resource.TestCheckResourceAttr("liviate_quota_tariff.test", "activation_rule", "serviceOffering.id == 'test-so-123' && zone.name == 'premium-zone'"),
				),
			},
		},
	})
