		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackCniConfigurationExists("liviate_cni_configuration.foo", &cniConfig),
					resource.TestCheckResourceAttr("liviate_cni_configuration.foo", "name", "test-cni-config"),
					resource.TestCheckResourceAttr("liviate_cni_configuration.foo", "params.#", "2"),
				),
			},
		},
	})
...
func testAccCheckCloudStackCniConfigurationDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "liviate_cni_configuration" {
			continue
		}
...
resource "liviate_cni_configuration" "foo" {
