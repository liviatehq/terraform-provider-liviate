		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackConfigurationExists("liviate_configuration.test", &configuration),
					resource.TestCheckResourceAttr("liviate_configuration.test", "value", "test_host"),
				),
			},
		},
	})
...
func TestAccCloudStackConfiguration_import(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				ResourceName:      "liviate_configuration.test",
				ImportState:       true,
...
func testAccResourceConfiguration() string {
	return fmt.Sprintf(`
	resource "liviate_configuration" "test" {
		name  = "host"
		value = "test_host"
	}
`)
