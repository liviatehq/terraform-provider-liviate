		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckCloudStackNetworkACLRulesExist("liviate_network_acl.foo"),
					resource.TestCheckResourceAttr(
						"liviate_network_acl_rule.foo", "rule.#", "4"),
					...
					resource.TestCheckTypeSetElemNestedAttrs(
						"liviate_network_acl_rule.foo", "rule.*", map[string]string{
							"rule_number":  "10",
							"action":       "allow",
							"protocol":     "all",
		...
		Steps: []resource.TestStep{
...

			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudStackNetworkACLRulesExist("liviate_network_acl.foo"),
					resource.TestCheckResourceAttr(
						"liviate_network_acl_rule.foo", "rule.#", "6"),
...
func testAccCheckCloudStackNetworkACLRuleDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "liviate_network_acl_rule" {
			continue
		}
...
resource "liviate_network_acl" "foo" {
  name = "terraform-acl"
  description = "terraform-acl-text"
  vpc_id = liviate_vpc.foo.id
}

resource "liviate_network_acl_rule" "foo" {
  acl_id = liviate_network_acl.foo.id
